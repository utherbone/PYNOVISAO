...
class GLCM(Extractor):
    ...
    def run(self, image):
        ...
        g = feature.graycomatrix(image_grayscale, [1, 2], [0, np.pi / 4, np.pi / 2], self.glcm_levels, normed=True, symmetric=True)

        contrast = feature.graycoprops(g, 'contrast').tolist()
        dissimilarity = feature.graycoprops(g, 'dissimilarity').tolist()
        homogeneity = feature.graycoprops(g, 'homogeneity').tolist()
        asm = feature.graycoprops(g, 'ASM').tolist()
        energy = feature.graycoprops(g, 'energy').tolist()
        correlation = feature.graycoprops(g, 'correlation').tolist()
